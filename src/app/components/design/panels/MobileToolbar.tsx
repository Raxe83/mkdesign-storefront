...
type ShapeAction = { icon: React.ReactNode; label: string; action: () => void };
...
export function MobileToolbar({ shapes, imageUploading, hasSelection, onUploadImage, onDelete, onDownload }: Props) {
  const tools = [
    ...shapes,
    {
      icon:   imageUploading ? <Loader2 size={16} className="animate-spin" /> : <ImagePlus size={16} />,
      label:  "Bild",
      action: onUploadImage,
    },
    { icon: <Trash2 size={16} />,   label: "Löschen",  action: onDelete,   destructive: true, disabled: !hasSelection },
