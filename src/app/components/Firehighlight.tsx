...
export interface FireHighlightFeature {
  text: string;
}
...
export interface FireHighlightProps {
  ...
  /** Bild links oder rechts – default: links */
...
export function FireHighlight({
  ...
  cta = {
    label: "Feuertonnen entdecken",
    href: "/collections/feuertonne",
  },
  image,
  imagePosition = "left",
  className,
  ...
  return (
    <ComponentLayout
      className={cn(
        "grid grid-cols-1 lg:grid-cols-2 min-h-[500px] bg-charcoal",
        className,
      )}
    >
      {/* ── Bild ── */}
      <div
        className={cn(
          "relative overflow-hidden min-h-[320px] lg:min-h-0",
          // Reihenfolge auf Mobile immer oben
          "order-first",
          // Auf Desktop: rechts wenn imagePosition="right"
      ...
      >
        ...
        {/* Subtiler Farbübergang zum Content-Bereich hin */}
        <div
          className={cn(
      ...
      >
        ...
        {/* Beschreibung */}
        {description && (
          <p className="text-white/60 text-sm leading-[1.85] mb-8 max-w-[44ch]">
            {description}
          </p>
        )}
