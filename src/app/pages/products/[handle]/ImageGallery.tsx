...
export default function ImageGallery({
  ...
  return (
    // Einzelner sticky-Container (top-16 = Höhe des fixed Headers, siehe
    // layout.tsx pt-16). Die Höhe ergibt sich normal aus dem Hauptbild
    // (aspect-[3/4]) — KEINE erzwungene volle Höhe, sonst wird das Bild auf
    // großen Screens unnötig riesig. Nur eine reine `max-height`-Bremse auf
    // dem Hauptbild selbst greift bei niedrigen Bildschirmen. Die Reserve
    // (18rem) muss den ungünstigsten Fall abdecken: VOR dem ersten Scrollen
    // sitzt die Galerie noch an ihrer natürlichen Position (spürbar tiefer
    // als der spätere Sticky-Punkt top-16, wegen "Zurück"-Button etc.
    // darüber) — reserviert also bewusst mehr als nur top-16 + Thumbnails,
    // sonst sind die Thumbnails vor dem ersten Scrollen abgeschnitten.
    <div className="flex flex-col gap-3 md:sticky md:top-16 w-full max-w-md mx-auto">
      {lightboxOpen && (
        <ReviewLightbox
          images={imageUrls}
      ...
      >
        {/* Main image */}
        <div
          className="relative aspect-[3/4] md:max-h-[calc(100vh-18rem)] w-full max-w-md mx-auto overflow-hidden rounded bg-zinc-100 dark:bg-zinc-800"
            ...
            >
              <Image
                ...
                alt={
                  ...
                  `${productTitle} – Bild ${currentIndex + 1}`
                }
                fill
                priority
          ...
          <div className="grid grid-cols-5 gap-2 shrink-0">
            {images.map((img, i) => (
              <button
                key={i}
                onClick={() => goTo(i)}
                aria-label={`${productTitle} – Bild ${i + 1}`}
